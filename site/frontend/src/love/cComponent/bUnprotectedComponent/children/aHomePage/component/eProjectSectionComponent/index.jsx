        ...
        <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
          ...
          <div className="flex mx-auto mb-10 justify-center">
            <ScrollArea className="w-80 lg:w-2/3 whitespace-nowrap rounded-md">
              {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
                <a 
                  ...
                  ${activeProjectGroup === each.title ? 
                    "bg-[#4A171E] dark:bg-[#E2B143] text-[#E2B143] dark:text-[#4A171E] border-[#E2B143] dark:border-[#4A171E] rounded-t" : 
              ...
              <ScrollBar orientation="horizontal" className="bg-[#4A171E] dark:bg-[#E2B143]" />
            </ScrollArea>
          </div>
          
          {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
            <React.Fragment key={index}>
              <div className={`flex flex-wrap text-center bg-[#4A171E] dark:bg-[#E2B143] text-[#E2B143] dark:text-[#4A171E] bg-opacity-60 px-4 py-6 rounded-lg ${activeProjectGroup === each.title ? "" : "hidden"}`}>
                ...
                {each?.projects?.map((each1, index1) => (
                  <React.Fragment key={index1}>
                    <div className="p-4 w-full md:w-1/3 text-left">
                      <div className="h-full bg-[#E2B143] dark:bg-[#4A171E] text-[#4A171E] dark:text-[#E2B143] rounded-lg overflow-hidden">
                          ...
                          <div className="flex items-center flex-wrap ">
                            {each1?.links?.map((each2, index2) => (
                              ...
                              each2?.title === "Visit Admin" ? (
                                <span key={index2} className="mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm py-1 border-r-2 border-[#4A171E] dark:border-[#E2B143]">
